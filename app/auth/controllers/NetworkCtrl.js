...
function connectToFacebook(models, token, refreshToken, profile, callback) {
	var User, Token, fbId;
	...
	async.waterfall([
		...
		function(callback) {
			...
			async.waterfall([
				function(callback) {
					AuthService.createNewUser(models, function(err, rNewUser) {
						if (err) {
							return callback(err);
						}
						User = rNewUser;
				...
				function(callback) {
					if (profile.emails && profile.emails.length) {
						...
						async.eachSeries(profile.emails, function(Email, callback) {
							AuthService.createNewEmail(models, User.userId, Email.value, function(err) {
								if (err) {
									return callback(err);
								}
								return callback();
		...
		function(callback) {
			AuthService.createNewToken(models, User.userId, function(err, rUser) {
				if (err) {
					return callback(err);
				}
				User = rUser;
